#!/usr/bin/env python3
"""
Enhanced report generator for Canvas API data
Fixes issues with student enrollment counting, emoji display, and excludes Precalculus
"""
...
def generate_email_html(grades_csv_path: str, template_path: str, output_path: str) -> None:
    ...
    # Exclude Precalculus from analysis as requested
    ...
    for course in course_columns:
        # IMPORTANT FIX: Filter out N/A and empty values
    ...
    # Estimated GPA (simplified calculation)
    average_gpa = 0
    grade_points_sum = 0
    enrollments_sum = 0
    
    for course in courses_data:
        # Approximate GPA contribution based on average score
        avg_score = float(course["avg_score"])
        if avg_score >= 90:
            grade_points = 4.0
        elif avg_score >= 80:
            grade_points = 3.0
        ...
        grade_points_sum += grade_points * course["student_count"]
        enrollments_sum += course["student_count"]
    
    if enrollments_sum > 0:
        average_gpa = round(grade_points_sum / enrollments_sum, 1)
    ...
    # Sort underperforming students by score (ascending) to prioritize the most concerning cases
    underperforming_students = sorted(underperforming_students, 
                                      key=lambda x: float(x["score"]))[:10]  # Limit to 10
    ...
    top_performers = sorted(top_performers, 
                            key=lambda x: float(x["score"]), reverse=True)[:10]  # Limit to 10
