#!/usr/bin/env python3
"""
Test script for generating fixed email reports
Using improved report generator that properly filters N/A students
"""
...
# Import our fixed generator instead of the original one
from emails.testing.fixed_report_generator import generate_email_html

def main():
    """Generate a test report from notion_grades.csv using the fixed generator"""
    # Define paths
    ...
    output_path = os.path.join(current_dir, "fixed_report.html")
    
    print(f"Generating fixed test report from {grades_csv_path}")
    print(f"Using template: {template_path}")
    print(f"Output will be saved to: {output_path}")
    
    # Generate the report with our fixed generator
    ...
    print("3. Improves emoji handling and display")
